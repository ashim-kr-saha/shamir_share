use shamir_share::{ShamirShare, Dealer};
...
fn main() {
    ...
    println!("Generated {} shares with indices: {:?}", 
             shares.len(), 
    ...
    for i in 1..=3 {
        if let Some(share) = dealer.next() {
            println!("Generated share {} with {} bytes of data", share.index, share.data.len());
        }
    }
    ...
    let even_shares: Vec<_> = shamir.dealer(secret)
        .filter(|share| share.index % 2 == 0)
        .take(5)
        .collect();
    
    println!("Generated {} even-indexed shares: {:?}", 
             even_shares.len(),
    ...
    println!("Reconstruction with even shares successful: {}", reconstructed == secret);
    println!();
    ...
    println!("Both methods produce equivalent results: {}", 
